...
def main(prob,group_norm,unet):
    ...
    test_acc_noisy1 = evaluate_model(model, loader_noise1, device, description=f"Final Test on Noisy Dataset (kernel={config.kernel1})")
    test_acc_noisy2 = evaluate_model(model, loader_noise2, device, description=f"Final Test on Noisy Dataset (kernel={config.kernel2})")
    wandb.run.summary["final_test_accuracy_clean"] = test_acc_clean
    wandb.run.summary["final_test_accuracy_kernel ={config.kernel1}"] = test_acc_noisy1
    wandb.run.summary["final_test_accuracy_kernel2 ={config.kernel2}"] = test_acc_noisy2
    # End the wandb run
