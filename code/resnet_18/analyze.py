...
if __name__ == "__main__":
    #model = load_model("gtsrb_motion_blur_resnet18_base_line.pth", group_norm=0, unet=False, models_location = str(Path(__file__).parent))
    #loader_clean, loader_noise1, loader_noise2 = get_test_loaders_for_motion_blur(batch_size=32, kernel_size1=51, kernel_size2=81, data_name="gtsrb")
    #device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    #model = model.to(device)
